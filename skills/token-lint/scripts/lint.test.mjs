...
test("segmentDistance: missing vapor prefix → too far", () => {
  assert.equal(
    segmentDistance("--foo-bar", "--vapor-color-red-100"),
    3,
  );
});
...
test("extractFromFile: emits --vapor-<group>-<name> for token leaf", async () => {
  await withTempAssets(async (dir) => {
    ...
    assert.deepEqual(
      [...out].sort(),
      ["--vapor-color-red-100", "--vapor-color-red-200"],
    );
  });
});
...
test("loadCanonical: merges multiple files", async () => {
  await withTempAssets(async (dir) => {
    ...
    assert.deepEqual(
      [...out].sort(),
      ["--vapor-size-space-100", "--vapor-typography-fontWeight-400"],
    );
  });
});
