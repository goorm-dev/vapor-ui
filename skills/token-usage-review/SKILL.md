...
description: >
  ...
  이 스킬은 vapor 토큰의 적법성(do-not-use·raw 미바인딩)과 의미 적합성(when/avoid 규칙)을
...
- **문법적 적법성 (결정론)**: do-not-use 토큰을 썼나? raw 색/raw 텍스트를 썼나(토큰 미사용)?
...
의미 조건이고, 순수 정적 분석으로 판정 가능한 건 일부(do-not-use, opacity, typography raw)뿐이다. 결정론으로
...
1. **통합 추출 (동결 스크립트)** — `/figma-use`를 로드한 뒤 `scripts/extract.figma.js`를 Read하고,
   상단 `__ROOT_ID__`·`__MODE__`만 치환해 `use_figma`에 read-only로 그대로 전달한다.
   **로직을 즉석 재작성하지 마라** — 재작성 드리프트가 정확도의 최대 위협이었다(결함 발견 시
   파일 자체를 고친다). 스크립트가 색상(alias 역추적)과 typography
   (`getStyledTextSegments` 분류)를 함께 뽑아 `{ colors, typography, stats }`를 그룹핑된
   형태(`nodeIds`/`count`)로 반환한다.
   - 반환 객체를 **Write로 파일에 저장**한다(예: `/tmp/token-review-extract.json`) — 2단의 두 스크립트가 같은 파일을 읽는다. echo 재출력 금지.
   - 추출 직후 셀프체크: 시안 결함 vs 추출 결함 구분(`extraction-and-resolution.md` §2, `typography-extraction.md` §4).

   → 색상 상세: **[references/extraction-and-resolution.md](references/extraction-and-resolution.md)** ·
   typography 상세: **[references/typography-extraction.md](references/typography-extraction.md)**.

2. **결정론 검사** — 추출 파일을 각 스크립트에 인자로 넘긴다.
   - 색상: `node scripts/evaluate.mjs <추출파일>` → do-not-use/unknown-token/미바인딩 위반.
   - typography: `node scripts/typography-evaluate.mjs <추출파일>` → `appliedStatus === 'raw'`만 high 위반. styled-override/var-only/mixed는 정상 취급.
3. **의미 판정 (LLM)** — 두 스크립트 출력의 **`rubric`**(입력에 실제 쓰인 토큰/Text Style의
   `intent`/`when`/`avoid` 서브셋)을 기준으로 판정한다. **스키마 파일 전체를 Read하지 마라.**
   typography는 스크린샷+의도로 위계 역할을 추론해 rubric과 대조. 각 판정에 confidence(HIGH/MED/LOW)를 단다.
4. **터미널 출력** — 색상·typography 위반을 **섹션 분리**해 출력. 적합률은 evaluate `summary`의 가중 집계값(색상 요소 + 텍스트 노드)을 통합. 되먹임은 제안까지만.

   → 2~4단계 상세(출력 템플릿 포함): **[references/evaluation-and-output.md](references/evaluation-and-output.md)**.

**스크린샷 분기**: `get_screenshot(maxDimension: 2576)`은 이미지 토큰이 크다. (a) typography 모드가 실행될 때(위계 추론에 필수), 또는 (b) 의도 미제공으로 역할 역추론이 필요할 때만 호출한다. "색상만 + 의도 제공" 실행이면 생략한다.
...
├── SKILL.md
├── references/
│   ├── extraction-and-resolution.md   # 색상 1단계: alias 역추적 / ColorElement 출력
...
└── scripts/
    ├── extract.figma.js               # 1단 통합 추출 동결 스크립트 (use_figma용 — ROOT_ID/MODE만 치환)
    ...
    ├── evaluate.mjs                   # 색상 결정론 (do-not-use/opacity/미바인딩 + 집계 가드 + rubric)
    ├── evaluate.test.mjs              # 색상 결정론 단위테스트 (node --test scripts/)
    ├── typography-evaluate.mjs        # typography 결정론 (raw=high, 나머지 정상 + 집계 가드 + rubric)
    └── typography-evaluate.test.mjs   # typography 결정론 단위테스트
```

> **결정론 임계값은 스키마 파싱이 아니라 코드 상수다.** disabled opacity 규칙은 `evaluate.mjs`의
> 상수(`DISABLED_OPACITY_PCT`)가 진실이다. typography 결정론은 `typography-evaluate.mjs`의
> `HIGH_STATUSES`가 진실(`raw`만).
...
> **1단계(통합 추출)도 번들 동결 스크립트다(`scripts/extract.figma.js`).** 에이전트는 `ROOT_ID`·`MODE`
> 두 상수만 치환해 `use_figma`에 전달한다 — 로직 즉석 재작성 금지. 불변 원칙 8개는 스크립트 상단
> 주석에 있고, 수정이 필요하면 그 원칙을 지켜 파일을 고치고 문서·테스트를 함께 갱신한다.

## 한계

LLM 의미 판정 confidence·component 역추적의 remote 변수/mode 의존성 등,
