...
// 전역 규칙(`_rules`)을 꺼낸다. v0.2.3 신버전에서 disabled opacity 규칙이 개별 토큰
// avoid가 아니라 여기로 모였다. 없으면 빈 객체.
...
// 주의: 이 스키마에는 프리미티브 hex가 없다(alias만 존재).
...
//
// 결정론 임계값(disabled opacity)은 더 이상 스키마 자연어/메타에서 파싱하지 않는다.
// 가짓수가 작고 거의 안 바뀌므로 evaluate 코드 상수로 옮겼다. 여기서 스키마에서 읽는 건
// do-not-use(status 열거값)·tokenKeys(키 집합)·_rules 키 목록(unknownGlobalRules 안전망)뿐
// — 모두 파싱이 아닌 키/열거값이다.
export function deriveRuleset(schema, globalRules) {
  ...
  // 미래 결정론 조건 감지. 지금 결정론으로 코드에 박힌 _rules 키는 disabled뿐.
  // 그 밖의 키가 등장하면 "결정론으로 다뤄야 할 새 규칙이 스키마에 들어왔는데 evaluate가
  // 모른다"는 신호다 — 값은 파싱하지 않고 키 목록만 보고 경고한다.
  const KNOWN_GLOBAL_RULES = new Set(["disabled"]);
  const unknownGlobalRules = Object.keys(globalRules).filter(
    (k) => !KNOWN_GLOBAL_RULES.has(k),
  );
