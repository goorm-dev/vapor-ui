...
import { SCHEMA_VERSION, loadGlobalRules, loadSchema, vaporMeta } from './schema-loader.mjs';
...
function parseRemedy(remedy) {
    ...
    const tokenPaths = [...remedy.matchAll(/colors\.[a-z0-9.]+/gi)].map((m) => m[0]);
...
async function main() {
    ...
    const disabledOpacityPercent = (() => {
        const m = String(globalRules.disabled?.rule ?? '').match(/apply\s+(\d+)%\s+opacity/i);
    ...
    const ruleset = {
        ...
        counts: {
            ...
            deterministicAvoid: avoidRules.filter((r) => r.classification === 'deterministic')
                .length,
            semanticAvoid: avoidRules.filter((r) => r.classification === 'semantic').length,
    ...
    if (unclassified.length) {
        ...
        for (const u of unclassified) console.log(`  [${u.token}] ${u.line}  (${u.reason})`);
    } else {
