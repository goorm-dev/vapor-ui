...
test("실제 스키마: foreground.*.100/200에 gradeRule이 채워진다", async () => {
  const { semantic } = await loadColorSchema("light");
  assert.match(
    semantic["colors.foreground.primary.100"].gradeRule,
    /pure white/i,
  );
  assert.match(
    semantic["colors.foreground.primary.200"].gradeRule,
    /not pure white/i,
  );
});
...
test("실제 스키마: light/dark가 같은 토큰 키 집합을 가진다", async () => {
  ...
  assert.deepEqual(
    ...
    Object.keys(dark.semantic).sort(),
  );
});
