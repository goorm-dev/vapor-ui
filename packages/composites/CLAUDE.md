...
- 모든 prop(시각 / 기능 / 슬롯)에 JSDoc을 작성한다. 이 내용을 그대로 Props Table 자동 생성에 사용한다.
...
2. 카테고리 배타성: `visual` / `functional` / `slot` 가운데 정확히 하나에만 속한다. 메타 정보는 prop 카테고리에 포함하지 않는다.
...
- 명시적인 지시가 없으면 `className` / `style` 오버라이드 prop을 추가하지 않는다. 오버라이드 정책은 아직 정하지 않았다.
