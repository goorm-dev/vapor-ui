import IconBase from '~/components/icon-base';
import type { IconProps } from '~/components/icon-base';

const FinlandColorIcon = (props: IconProps) => (
    <IconBase viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
        <mask
            id="mask0_9214_1307"
            style={{ maskType: 'luminance' }}
            maskUnits="userSpaceOnUse"
            x="0"
            y="0"
            width="16"
            height="16"
        >
            <path
                d="M8 16C12.4183 16 16 12.4183 16 8C16 3.58172 12.4183 0 8 0C3.58172 0 0 3.58172 0 8C0 12.4183 3.58172 16 8 16Z"
                fill="white"
            />
        </mask>
        <g mask="url(#mask0_9214_1307)">
            <path d="M5.06561 9.70911H-2.27441V15.9091H5.06561V9.70911Z" fill="white" />
            <path d="M5.06561 0.0908813H-2.27441V6.29088H5.06561V0.0908813Z" fill="white" />
            <path d="M23.7255 9.70911H9.11279V15.9091H23.7255V9.70911Z" fill="white" />
            <path d="M23.7255 0.0908813H9.11279V6.29088H23.7255V0.0908813Z" fill="white" />
            <path
                d="M8 16.1818C3.48909 16.1818 -0.181816 12.5109 -0.181816 7.99999C-0.181816 3.48909 3.48909 -0.181824 8 -0.181824C12.5109 -0.181824 16.1818 3.48909 16.1818 7.99999C16.1818 12.5109 12.5109 16.1818 8 16.1818ZM8 0.181813C3.68909 0.181813 0.18182 3.68909 0.18182 7.99999C0.18182 12.3109 3.68909 15.8182 8 15.8182C12.3109 15.8182 15.8182 12.3109 15.8182 7.99999C15.8182 3.68909 12.3109 0.181813 8 0.181813Z"
                fill="#F2F2F0"
            />
            <path
                d="M9.27979 0H4.89609V5.81818H-2.36572V10.1818H4.89609V16H9.27979V10.1818H23.8161V5.81818H9.27979V0Z"
                fill="#002F6C"
            />
        </g>
    </IconBase>
);

export default FinlandColorIcon;
