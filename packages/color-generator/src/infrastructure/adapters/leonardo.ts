...
// Leonardo 라이브러리의 타입을 직접 사용하므로 커스텀 타입 정의 제거
 ...
 * 토큰 이름을 생성합니다.
 */
const generateTokenName = (parts: string[]): string => {
 ...
 * 코드 문법용 이름을 생성합니다.
 */
const generateCodeSyntax = (parts: string[]): string => {
 ...
 */
const extractBackgroundCanvas = (backgroundObj: ContrastColorBackground): BackgroundCanvas => {
 ...
 */
const convertToPrimitivePalette = (
    leonardoColor: ContrastColor,
    ...
    leonardoColor.values.forEach((instance) => {
...
/**
 * Leonardo Adapter 구현체
 * ColorGeneratorPort 인터페이스를 구현하는 함수형 객체입니다.
 */
export const leonardoAdapter: ColorGeneratorPort = {
    generatePalette,
};
