...
const [VaporLegacyIdProvider, useVaporLegacyIdCounter] = createContext<React.MutableRefObject<number>>({
...
function useCounter(isDisabled = false) {
    ...
    if (ref.current === null && !isDisabled) {
        ...
        // @ts-ignore
        const currentOwner = (React as AnyProp).__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED?.ReactCurrentOwner?.current;
        if (currentOwner) {
