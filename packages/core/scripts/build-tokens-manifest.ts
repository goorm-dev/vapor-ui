...
const SRC = resolve(here, '..', 'src', 'styles', 'tokens');
...
const SOURCES: SourceEntry[] = [
    { file: 'color/basic-color.ts', exportName: 'LIGHT_BASIC_COLORS', scope: 'color', cssVarPrefix: ['color'] },
    {
...
if (mode === 'src' || mode === 'both') {
    const generatedPath = resolve(here, '..', 'src', '$style.tokens.generated.ts');
    const generatedContent = `// AUTO-GENERATED by scripts/build-tokens-manifest.ts. Do not edit.
...
if (mode === 'dist' || mode === 'both') {
    ...
    const outPath = resolve(outDir, 'tokens.manifest.json');
