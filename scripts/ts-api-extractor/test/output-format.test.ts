...
describe('formatFileName', () => {
    it('컴포넌트명 → kebab-case.json', () => {
        expect(formatFileName('Button', jsonOutputFormat)).toBe('button.json');
    });

    it('복합 컴포넌트명', () => {
        expect(formatFileName('CollapsibleRoot', jsonOutputFormat)).toBe('collapsible-root.json');
    });

    it('세 단어 이상', () => {
        expect(formatFileName('DataTableHeader', jsonOutputFormat)).toBe('data-table-header.json');
    });
});
