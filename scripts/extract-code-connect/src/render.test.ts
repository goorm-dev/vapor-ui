...
`);
    ...
    it('index.parts.ts 가 없으면 flat 이름, slot 이 없으면 self-closing', async () => {
        ...
        const out = await fmt(
            render({ ...base, blocks, componentName: 'Badge', kebab: 'badge', hasParts: false }),
        );
    ...
    it('utilsImport 와 packageImportPath 를 그대로 쓴다', async () => {
        ...
        const out = await fmt(
            render({
                ...
                hasParts: false,
                utilsImport: '../../../utils/figma-utils',
                packageImportPath: '@vapor-ui/core',
            }),
        );
        expect(out).toContain("import { getProperties } from '../../../utils/figma-utils';");
        expect(out).toContain('import { Button } from "@vapor-ui/core"');
    });

    it('INSTANCE_SWAP todo 는 블록 안 주석으로 남는다', async () => {
        const blocks: Block[] = [
            { varName: 'slot', instanceName: '(Slot)', entries: {}, todos: ['Icon'] },
        ];
        const out = await fmt(
