...
import { DEFAULT_COLOR, rasterize } from './render';
...
const renderInBrowser = async (name: string, color = DEFAULT_COLOR) => {
    const Icon = icons[name];
    assert.equal(typeof Icon, 'function', `${name} is not exported from the icons bundle`);
    const markup = renderToStaticMarkup(
        createElement(Icon, { width: SIZE, height: SIZE, style: { color } }),
    );
...
const fill = await renderInBrowser('AchievementIcon');
const fillAgain = await renderInBrowser('AchievementIcon');
const outline = await renderInBrowser('AchievementOutlineIcon');
...
const sparseBlack = await renderInBrowser('IntelliSenseNullIcon');
const sparseRed = await renderInBrowser('IntelliSenseNullIcon', '#f00');
await browser.close();
