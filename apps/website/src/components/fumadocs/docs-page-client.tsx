...
import { LayoutContext, isActive } from './docs-layout-client';
...
export function PageTOCPopoverTrigger({ className, ...props }: ComponentProps<'button'>) {
    ...
    return (
        ...
        >
            <ProgressCircle
                className={clsx('shrink-0', open && 'text-fd-primary')}
                max={1}
            ...
            <ChevronDownOutlineIcon
                className={clsx('mx-0.5 shrink-0 transition-transform', open && 'rotate-180')}
            />
        </CollapsibleTrigger>
    );
}
...
export function PageTOCPopoverContent(props: ComponentProps<'div'>) {
    return (
        <CollapsibleContent
            ...
            {...props}
            className={clsx('flex max-h-[50vh] flex-col px-4 md:px-6', props.className)}
        >
            {props.children}
        </CollapsibleContent>
    );
    ...
    return (
        ...
        >
            {items.map((item, i) => {
                ...
                return (
                    <Fragment key={i}>
                        {i !== 0 && <ChevronRightOutlineIcon className="size-3.5 shrink-0" />}
                        {item.url ? (
                            <Link
