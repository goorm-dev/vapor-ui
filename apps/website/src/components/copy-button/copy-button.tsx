...
const openLLMChat = (llmType: 'claude' | 'chatgpt', docUrl: string) => {
    const prompt = encodeURIComponent(`${docUrl} 문서를 읽고 질문에 답해줘.`);
    const urls = {
        claude: `https://claude.ai/new?q=${prompt}`,
        chatgpt: `https://chat.openai.com/?q=${prompt}`,
    };

    window.open(urls[llmType], '_blank');
};
...
export const CopyButton = ({ markdownUrl }: CopyButtonProps) => {
    ...
    const menuItems = useMemo(
        () => [
            {
                label: '마크다운으로 보기',
                icon: <CopyAsMarkdownOutlineIcon width={16} height={16} />,
                onClick: () => window.open(markdownUrl, '_blank'),
                isExternal: true,
            },
            {
                label: 'Claude에게 질문하기',
