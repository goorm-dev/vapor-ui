...
| `themes`        | `('light' \| 'dark' \| 'system')[]`                                            | 사용 가능한 테마 목록. `enableSystem`이 `true`일 때 `['light', 'dark', 'system']`, `false`일 때 `['light', 'dark']`.  |
| `resolvedTheme` | `'light' \| 'dark' \| undefined`                                               | 실제로 적용된 테마. `theme`이 `'system'`일 때 현재 시스템 테마를 반영합니다.                        |
| `systemTheme`   | `'light' \| 'dark' \| undefined`                                               | 현재 사용자의 시스템 테마. `enableSystem`이 `false`일 때는 `undefined`.                             |
...
`defaultTheme="system"`으로 설정하거나 사용자가 `setTheme('system')`을 호출하면 운영체제 테마 설정을 자동으로 감지합니다:

- **macOS**: 시스템 환경설정 > 일반 > 외관 모드
- **Windows**: 설정 > 개인 설정 > 색
- **Linux**: 데스크톱 환경의 테마 설정

내부적으로 `prefers-color-scheme` CSS 미디어 쿼리를 사용하여 실시간으로 변경사항을 감지하고 자동으로 테마를 전환합니다. 단, `enableSystem={false}`로 설정하면 초기 로드 시에만 참조하고 이후에는 동기화되지 않습니다.
...
3.  **`defaultTheme`**: 초기 로드용 기본 테마 (`localStorage`에 저장된 값이 없을 때만 사용)
4.  **시스템 테마**: `defaultTheme`이 `'system'`일 때만 참조되는 시스템 환경 설정
...
```tsx
import type { ThemeConfig, UseThemeProps, ThemeScopeProps } from '@vapor-ui/core';
