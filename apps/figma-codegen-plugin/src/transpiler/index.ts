...
import { loadMetadata, validateMetadata } from '../infrastructure/metadata';
...
export interface TranspilerOptions {
    ...
    /**
     * 메타데이터 파일 경로 (optional)
     * Phase 2: 메타데이터 기반 보강 활성화
     */
    metadataPath?: string;

    /**
     * 메타데이터 객체 (직접 전달)
     * metadataPath보다 우선순위가 높음
     */
...
export async function createTranspiler(
    ...
    // 1. 메타데이터 로드 (Phase 2)
    let metadata: ComponentMetadata | undefined = options.metadata;
    ...
    if (!metadata && options.metadataPath) {
        try {
            ...
            if (!validation.valid) {
                ...
                console.warn('Falling back to default metadata');
                metadata = await loadMetadata(); // 기본 메타데이터 사용
            }
        } catch (error) {
