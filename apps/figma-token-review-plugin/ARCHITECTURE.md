...
- **`code.ts`** *(entry)* — UI 표시 + 크기 복원 + 핸들러 init + bus.start. 15줄.
...
- **`call-evaluator.ts`** — 토큰 검수 실행기 호출. scan 핸들러 의존.
...
| `scan.ts` | `activeRequestId`로 in-flight scan cancel, `call-evaluator` 호출, 결과 echo |
...
- **`main.tsx`** *(entry)* — `<StrictMode> → <ErrorBoundary> → <Providers> → <App>`. Render 트리만 조립.
