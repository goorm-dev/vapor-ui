...
- JS/TS/JSX/TSX 파일만 검사 (ESLint 기본 적용 범위).
- 검사 컨텍스트 4종:
...
- `.css`, `.scss`, `.less` 등 순수 스타일시트 파일 (ESLint 대상이 아님; stylelint 분리).
...
│   │   ├── style-context.ts            # NEW: JSX style obj / template / className walker
│   │   ├── get-source.ts
│   │   └── guard.ts
│   ├── generated/
│   │   └── tokens.ts                   # GENERATED, .gitignore 대상
│   ├── types/
│   └── index.ts                        # 룰 등록 + recommended preset 갱신
...
- canonical 토큰 그대로 사용
- `--vapor-*` 미포함 string
- `allowCustomTokens: ['--vapor-app-*']` 매치
...
- Rules 표에 신규 룰 2개 + 옵션 예시 추가
- 토큰 SSOT 위치 명시 (`skills/token-lint/assets`, build-time 추출)

CHANGELOG.md: 직접 작성하지 않음 (changesets 가 관리).

## 10. 단계별 구현 순서 (요약, plan 단계에서 상세화)

1. `scripts/extract-tokens.mjs` + `src/generated/tokens.ts` 생성 검증
2. `utils/token-segment-distance.ts`, `utils/property-category.ts`, `utils/style-context.ts`, `utils/token-string.ts`
3. `rules/no-invalid-design-token.ts` + 테스트
4. `rules/prefer-design-token.ts` + 테스트
5. `index.ts` 등록 + recommended preset 갱신
6. README 갱신
7. 빌드 / lint / 타입체크 / 테스트 전체 통과 확인
